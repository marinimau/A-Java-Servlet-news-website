...
public class searchAjax extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(command != null && command.equals("cerca"))
        {
            String query_string = request.getParameter("query");
            
            System.out.println("L'utente ha digitato: " + query_string);
            
            ArrayList<Articolo> all_news = ArticoloFactory.getInstance().cercaArticolo(query_string);
            
            if(all_news.size()>0)
                request.setAttribute("notizie", all_news);
            ...
            request.getRequestDispatcher("M1/notiziaTrovata_JSON.jsp").forward(request, response);
        }
    }
